    ...
    if ([self validateValue:value forKey:keyPath])
    {
        ...
        } else
        {
            ...
            if (object) {
                ...
                [object setValueSafe:value forKey:lastKey];
            }
        }

        if (!isHandle) {
            NSString* message = [[NSString alloc] initWithFormat:@"[setValueSafe:keyPath]-[setValue:forUndefinedKey:]: this class is not key value coding-compliant for the keyPath %@.",keyPath];
            [[HYJADCrashCollectManager shared] commitCrashLog:message];
        }
    }
}
...
{//
    NSString* message = [[NSString alloc] initWithFormat:@"[valueForKey:]: this class is not key value coding-compliant for the key %@.",key];
    [[HYJADCrashCollectManager shared] commitCrashLog:message];
    return nil;
}
...
{//
    ...
    {
        NSString* message = [[NSString alloc] initWithFormat:@"[valueForKeyPath:]: keyPath is not null"];
        [[HYJADCrashCollectManager shared] commitCrashLog:message];
        return nil;
    }
    ...
    } else
    {
        ...
        if (object) {
            NSString *lastKey = [keyPathArray lastObject];
            return [object valueForKey:lastKey];
        }
    }
    
    NSString* message = [[NSString alloc] initWithFormat:@"[valueForKeyPath:]: this class is not key value coding-compliant for the keyPath %@.",keyPath];
    [[HYJADCrashCollectManager shared] commitCrashLog:message];
    return nil;
}
